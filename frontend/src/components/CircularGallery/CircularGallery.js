...
class Media {
    ...
    createShader() {
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          // Wave animation removed for flat display
          p.z = 0.0;
...
class App {
    ...
    onCanvasClick(e) {
        if (!this.onImageClick || !this.medias) return;

        // Find the media item closest to center
        ...
        if (closestMedia && closestDistance < 3) {
            const itemIndex = closestMedia.index % this.items.length;
            this.onImageClick(this.items[itemIndex]);
        }
    }
    addEventListeners() {
